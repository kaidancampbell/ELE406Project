...
HASH:
	...
	;REPLACING THE POINTER TO m1024
	ldi r5, 0
	mov r7, r5
	;ldi	r7, w80
	;IN THIS SECTION r7 MEANS w80
	ldi r2, 16

sch1:
	ldix	r0, r5, m1024	;get the current address for M
	stix	r5, r0, w80 
...
sch2:
	...
	sig1	;r0&r1 = sig1(W(t-2))
	
	adi	r5, 0xFFF6	;t-7 (actually t-14)
	adi r7, 0xFFF6
	...
	adi	r5, 0xFFF0 ;(t-15) actually t-30 because of increased word length 
	adi r7, 0xFFF0


	ldix r0, r5, w80 
	ldix r1, r7, w80	;r0&r1 = W(t-15)
	sig0 ;r0&r1 = sig0(W(t-15))

	adi	r5, 0xFFFE	;(t-16) actually t-32 because yadda yadda 
	adi r7, 0xFFFE
	;adi r7, 1
	...
	add64	r0, r1, r6, r7  ;r0&r1 = sig1(W(t-15))+W(t-16)
	add64 r3, r4, r0, r1	;r3&r4 = sig1(W(t-2))+W(t-7)+sig1(W(t-15))+W(t-16) 
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	;R5 CONTAINS THE REFERENCE TO HINIT
	ldi	r5, 0
	; R6 CONTAINS THE REFERENCE TO WVA
	ldi r6, 1
	; R7 CONTAINS THE REFERENCE TO h0
	ldi	r3, 8
...
sch4:
	...
	ldm	r3, wvh
	ldm r4, wvhl
	add64	r0, r1, r3, r4 ;r0&r1 = h+sum1
	push r0
	push r1
	...
	pop	r4	
	pop	r3	;r3&r4 = h+sum1
	add64	r0, r1, r3, r4 	;r0&r1 = h+sum1+ch(r0&r1, r2&r3, r3&r4)
	mov	r3, r0
	mov r4, r1
	...
	ldix	r0, r6, k512
	ldix	r1, r7, k512
	add64	r0, r1, r3, r4 	;r0&r1 = h+sum1+ch(r0&r1, r2&r3, r3&r4 )+ K
	mov	r3, r0
	mov r4, r1
	...
	add64	r0, r1, r3, r4 	;r0&r1 = h+sum1+ch(r0&r1, r2&r3, r3&r4 )+ K + W
	...
	sum0		;creating sum0 and storing
	...
	maj	;producing maj in r0&r1

	pop	r4		;recallling sum0 into r4 and r3
	pop	r3

	add64	r0, r1, r3, r4 	;r0&r1 = sum0+maj
	...
	add64	r0, r1, r3, r4  ;r0&r1 = t1+t2

	stm	wva, r0		;store t1+t2 in a
	stm	wval, r1

	adi	r6, 2		;increment r6
	adi r7, 2		;increment r7
...
sch5:
	...
	add64	r0, r1, r3, r4 	;r0&r1 = wva + h0


	stix	r6, r0, h0
	stix	r7, r1, h0 
	
	adi	r6, 2		;increment pointers
	adi r7, 2
